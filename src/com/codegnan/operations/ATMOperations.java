...
import com.codegnan.customExceptions.IncorrectPinException;
...
public class ATMOperations {
	...
	public static IATMService validCredentials(long cardNumber, int pinNumber)
			throws InvalidCardException, InvalidPinException, IncorrectPinException {
		if (dataBase.containsKey(cardNumber)) {
		...
		if (card.getChances() >= 3) {
			throw new IncorrectPinException("you have reached wrong limit of pin number which is 3  attempts ");
		}
		if (card.getPinNumber() != pinNumber) {
	...
	public static void main(String[] args) throws NotAOperatorException{
		...
		while(MACHINE_ON) {
			...
			}catch(InvalidPinException|InvalidCardException|IncorrectPinException e) {
